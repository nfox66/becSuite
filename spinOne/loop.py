...
def setPhi(phiOne,phiZero,phiMinusOne,ix,iy):
    ...
    phiList = np.zeros(3,dtype='complex_')
    phiList[0] = phiOne[iy,ix]
    phiList[1] = phiZero[iy,ix]
    phiList[2] = phiMinusOne[iy,ix]
...
def getPeriodicCirclePhis(x,y,yPoint,n,phiOne,phiZero,phiMinusOne):
    ...
    for i in range(n):
        phiList[i] = setPhi(phiOne,phiZero,phiMinusOne,xr[i],yIndex)
...
def getCirclePhis(r,loopPts,xpts,ypts,phiOne,phiZero,phiMinusOne):
    ...
    def approximatePhi(xr,yr,xIndex,yIndex,phiOne,phiZero,phiMinusOne):
        ...
        if (len(xIndex) == 1 and len(yIndex) == 1):
            ...
            phiList = setPhi(phiOne,phiZero,phiMinusOne,xIndex[0],yIndex[0])
        elif(len(xIndex) == 1):
            """Loop point between two grid points, return weighted average of condensate values."""
            phiA = setPhi(phiOne,phiZero,phiMinusOne,xIndex[0],yIndex[0])
            phiB = setPhi(phiOne,phiZero,phiMinusOne,xIndex[0],yIndex[1])
            ...
            phiA = setPhi(phiOne,phiZero,phiMinusOne,xIndex[0],yIndex[0])
            phiB = setPhi(phiOne,phiZero,phiMinusOne,xIndex[1],yIndex[0])
            ...
            phiA = setPhi(phiOne,phiZero,phiMinusOne,xIndex[0],yIndex[0])
            phiB = setPhi(phiOne,phiZero,phiMinusOne,xIndex[0],yIndex[1])
            phiC = setPhi(phiOne,phiZero,phiMinusOne,xIndex[1],yIndex[0])
            phiD = setPhi(phiOne,phiZero,phiMinusOne,xIndex[1],yIndex[1])
    ...
    for i in range(loopPts):
        ...
        phiList[i] = approximatePhi(xr[i], yr[i], xIndex, yIndex, phiOne, phiZero, phiMinusOne)
